/**
 * <closer-click-share> — UI compartida del ecosistema CloserClick para COMPARTIR
 ...
 *
 * Filosofía CloserClick: autohosteado, Shadow DOM, sin JS de terceros ni cookies,
...
class CloserClickShare extends HTMLElement {
  static get observedAttributes () { return ['url', 'text', 'lang', 'heading', 'hint', 'open'] }
  ...
  _shareTo (key) {
    ...
    else if (key === 'more') this._shareNative()
  }
  async _shareNative () {
    if (typeof navigator.share === 'function') {
      try { await navigator.share({ title: this._shareText, text: this._shareText, url: this._url }) } catch (_) {}
    } else { this._copy() }
  }
...
if (typeof customElements !== 'undefined' && !customElements.get('closer-click-share')) {
  customElements.define('closer-click-share', CloserClickShare)
}

export { CloserClickShare }
